...
int lua_cocos2dx_net_Net_sendProto(lua_State* tolua_S)
{
    ...
    if (argc == 2) 
    {
        ...
        ok &= luaval_to_int32(tolua_S, 3,(int *)&arg1, "Net:sendProto");
        if(!ok)
        {
...
int lua_cocos2dx_net_Net_reConnect(lua_State* tolua_S)
{
    int argc = 0;
    Net* cobj = nullptr;
    bool ok  = true;
    ...
    if (!cobj) 
    {
        tolua_error(tolua_S,"invalid 'cobj' in function 'lua_cocos2dx_net_Net_reConnect'", nullptr);
        return 0;
    }
#endif
    ...
    if (argc == 0) 
    {
        if(!ok)
        {
            tolua_error(tolua_S,"invalid arguments in function 'lua_cocos2dx_net_Net_reConnect'", nullptr);
            return 0;
        }
        cobj->reConnect();
        lua_settop(tolua_S, 1);
    ...
    luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d \n", "Net:reConnect",argc, 0);
    return 0;

#if COCOS2D_DEBUG >= 1
    tolua_lerror:
    tolua_error(tolua_S,"#ferror in function 'lua_cocos2dx_net_Net_reConnect'.",&tolua_err);
#endif
...
int lua_register_cocos2dx_net_Net(lua_State* tolua_S)
{
    ...
    tolua_beginmodule(tolua_S,"Net");
        ...
        tolua_function(tolua_S,"reConnect",lua_cocos2dx_net_Net_reConnect);
