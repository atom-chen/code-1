...
function UpdateView:startGame()
  ...
  return result
    ...
    if allPath ~= "" then
        ...
        if oldPaths then
            oldPaths = oldPaths .. "#"
            local newPaths = removeRepeatData(HotUpdateUtils.split(oldPaths..allPath, "#")) 
            local savePaths = HotUpdateUtils.otherSerialize(newPaths, "#")
            HotUpdateUtils.writeDataByKey("allPath", savePaths)
        else
    ...
    if paths and paths ~= "" then
        for k,v in pairs(HotUpdateUtils.split(paths, "#")) do
            if v ~= "" then
                print("添加搜索路径---->",v)
