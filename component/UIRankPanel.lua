...
function UIRankPanel:initPanel(info)
    ...
    -- rewardPanel:setLocalZOrder(1)
    
	-- if rewardPanel.layoutChild == nil then
 --        local layout = ccui.Layout:create()
 --        layout:setContentSize(cc.size(3000, 3000))
 --        local winSize = rewardPanel:getContentSize()
 ...
 --        rewardPanel:addChild(layout, -1)
 --        -- self._uiSkin:addChild(layout)
 --        layout:setTouchEnabled(true)
 --        self._panel:addTouchEventListener(layout, function()
	-- 	    rewardPanel:setVisible(false)
	--     end)
 --        rewardPanel.layoutChild = layout
 --    end
    ...
    if self.secLvBg == nil then
	    ...
	    -- self.secLvBg = UISecLvPanelBg.new(rewardPanel, self, extra)
	    ...
	    -- self._uiSkin:setLocalZOrder(1000)
    ...
    -- -- 关闭按钮响应
    -- local closeBtn = self:getChildByName("Panel_14/rewardPanel/frameTop/closeBtn")
    -- self._panel:addTouchEventListener(closeBtn, function()
	   --  rewardPanel:setVisible(false)
    -- end)
...
function UIRankPanel:updateData(info)  --刷新listview数据
	...
	if info.tipText ~= nil then
		self._panel:addTouchEventListener(self.tip_btn, function()
			...
			local text = {{{content = info.tipText, foneSize = ColorUtils.tipSize20, color = ColorUtils.wordColorDark1601}}}
	...
	self:renderListView(self.listView, data.activityRankInfos, self, self.renderItem)
end
