...
--[[
    ...
    GameConfig.battleType.rebels = 11 --消灭叛军
]]
...
function UITeamDetailPanel:ctor(parent,data,type,callBack,extra)
    ...
    self._extra = rawget(data,"extra") or {}   --配置显示参数(星星，战损，挂机)，默认都显示
...
-- 每次打开刷新数据
function UITeamDetailPanel:onUpdateData(data, flag)    
    ...
    if self._type == GameConfig.battleType.legion then  --军团副本校验怪物数量变化
        if self._legionMonsterInfos ~= nil then
            ...
            for k,v in pairs(data.monsterInfos) do
                ...
                if v.num ~= self._legionMonsterInfos[k].num 
                    or v.id ~= self._legionMonsterInfos[k].id
                    or v.post ~= self._legionMonsterInfos[k].post then
                    isDifferent = true
                end
            end
...
function UITeamDetailPanel:setCurrFightR(data)  
    local fightWight = self._panelR:getChildByName("fightWight")
    local fightImg = self._panelR:getChildByName("fightImg")
    fightWight:setVisible(false)
...
function UITeamDetailPanel:showLostInfo( isShow, count )
    ...
    -- print("战损 count,_type,_sendType=", count, self._type, self._sendType)
...
function UITeamDetailPanel:isCanDo()
    ...
    if currentType then
        ...
        elseif currentType == 2 then
            if dungeoId == 1 or dungeoId == 2 or dungeoId == 3 then --1=匈奴,2=鲜卑,3=南越
                -- 挑战次数不足，不可以挂机
...
function UITeamDetailPanel:onTouchFightBtnHandle(sender) --出战按钮
    ...
    -- data.type = self._sendType
    -- data.id = self._cityId
    -- -- data.infos = self:checkFightPosMap()
...
function UITeamDetailPanel:onBuyTimesResp(data)
    -- print("购买次数 UITeamDetailPanel onBuyTimesResp")
    if self._parent:isVisible() ~= true then
        -- print("UITeamDetailPanel 没显示 ")
        return
    end

    -- local sender = self._sleepBtn
    ...
    local function callbk()
        local function callFunc()
            ...
            -- function callreq2()
            --     self:dispatchEvent(DungeonEvent.BUY_ENERGY, 2)
            -- end
            if data.dungeoId == 1 or data.dungeoId == 2  then     
...
function UITeamDetailPanel:getSoliderPosCount(num, pos, heroBtnType)
    local color
    local curNum = num
...
function UITeamDetailPanel:setPuppetById(pos,id,num, isCalFightPosMap)
    ...
    -- print("我方坑位佣兵 pos,id,num=",pos,id,num)
    local team = self:getTeamByPos(pos)
    team:setScale(GlobalConfig.UITeamDetailScale)
...
function UITeamDetailPanel:setCurrFight()  --战力显示
    local fightWight = self._panelL:getChildByName("fightWight")
    local fightImg = self._panelL:getChildByName("fightImg")
    -- if self._currFight == 0 then
    --     self:addButtonAction()
    -- else
    --     if self._effImg ~= nil then
    --         self._effImg:setVisible(false)
    --     end
    -- end
    -- fightWight:setString(StringUtils:formatNumberByK3(self._currFight))
...
function UITeamDetailPanel:setFirstNumber(addnum) --先手值
    ...
    local number = self._soliderProxy:getTotalFirstnum() + addnum
...
--    for i=1,6 do
--        local heroData = heroProxy:getHeroInfoWithPos(i)
--        local command = heroProxy:getHeroCommandNumWithData(heroData)
--        count = count + command
--    end
...
function UITeamDetailPanel:updateConsuPos(data)
    ComponentUtils:updateConsuPos(self._consuL,data)
end
