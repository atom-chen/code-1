...
function UIResourceBuy:renderOneItem(itemPanel, info, index)
    ...
    if icon == nil then
        ...
        -- icon = UIIcon.new(iconContainer, iconInfo, true)
    ...
    -- infoStr = StringUtils:getStringAddBackEnter(infoStr,12) --自动换行
    infoTxt:setString(infoStr)
    priceTxt:setString(info.price)
    -- numTxt:setString(string.format(TextWords:getTextWord(103),info.curNum))
    -- numTxt:setString(info.curNum)
...
function UIResourceBuy:onItemBtnTouch(sender)
    ...
    if sender.type == 0 then -- 使用
        ...
        -- local itemData = {}
        -- itemData.curNum,itemData.name,itemData.info,itemData.itemID = sender.curNum,sender.name,sender.info,sender.itemID
        -- self:addMoveBtn(itemData)
