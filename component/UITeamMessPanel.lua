...
function UITeamMessPanel:ctor(panel,data,pos,callBack,consuData)
    ...
    self:updateData(data,pos,consuData)
...
function UITeamMessPanel:updateData(data,pos,consuData)
    consuData = consuData or {}
    local command = 0
    -- if type(consuId) == "number" then
    --     command = self._soliderProxy:getAdviserCommand(consuData)
    -- end
    command = self._soliderProxy:getAdviserCommand(consuData)
