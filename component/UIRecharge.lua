...
function UIRecharge:initPanel()
    ...
    if vipexp == 0 then
        -- 首冲
        self._secLvBg:setContentHeight(480)
        panel:setVisible(true)
        self:initReardPanel(panel)
    else
        -- 非首冲
        self._secLvBg:setContentHeight(370)
...
function UIRecharge:initReardPanel(panel)
    -- 首冲礼包数据
    local rewardList = {
        {typeid = 24, num = 1, power = 409, isShowNum = true},
        {typeid = 1052, num = 1, power = 401, isShowNum = true},
        {typeid = 3311, num = 1, power = 401, isShowNum = true},
        {typeid = 206, num = 0, power = 407, isShowNum = false},
    }
