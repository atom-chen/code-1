...
function UICityInfoPanel:updateTopInfo(data)
    ...
    local color = myFight > force and cc.c3b(0,255,0) or cc.c3b(255,0,0)

    force = StringUtils:formatNumberByK3(force)
    self.fightTxt:setString(force)
    self.fightTxt:setColor(color)

    local targetTxt = self._uiSkin:getChildByName("mainPanel/topBgImg/targetTxt")
    targetTxt:setColor(color)
...
function UICityInfoPanel:renderItem(data)
    for i=1,4 do
        ...
        item:getChildByName("nameTxt"):setVisible(false)
...
-- -- 通过type读取配表数据
-- function UICityInfoPanel:getConfigByType(type)
--     if self.configTab == nil then
--         self.configTab = {}
--         self.configTab[1] = {nil,nil,"AdventureEventConfig","rewardshow"}                           --匈奴
--         self.configTab[2] = {"AdventureEventConfig","exp","AdventureEventConfig","rewardshow"}      --鲜卑
--         self.configTab[5] = {"EventConfig","exp","EventConfig","rewardshow"}                        --中原战役
--         self.configTab[6] = {"LegionEventConfig","fixchandrop","LegionEventConfig","fixkilldrop"}   --军团
--         self.configTab[9] = {"PanditMonsterConfig","fixreward","PanditMonsterConfig","rewardshow"}  --剿匪
--     end
--     return self.configTab[type]
-- end
...
function UICityInfoPanel:updateDataByType2(data)
    ...
    local rewards = StringUtils:jsonDecode(data.rewardshow)
    for _,reward in pairs(rewards) do
        print("............ 鲜卑 ",reward[1],reward[2],reward[3])
        local info = {}
        info.power = reward[1]
        info.typeid = reward[2]
        info.num = reward[3]
...
function UICityInfoPanel:updateData(data,citytype)
    ...
    print("......................updateData",self.type,type,citytype,self._star)

    local fun = self["updateDataByType"..self.type]
    if fun and table.size(cityInfo) > 0 then
        fun(self,cityInfo)
    end
