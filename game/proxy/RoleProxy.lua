...
function RoleProxy:ctor()
    ...
    self._maxCrusadeEnergy = 10 --讨伐令最大值
...
function RoleProxy:initSyncData(data) --2万协议
    ...
    if actorInfo.tanbaoFrees then
        ...
        lotteryProxy:setIsFreeTanbao(actorInfo.tanbaoFrees[1],actorInfo.tanbaoFrees[2])
    end
...
function RoleProxy:onTriggerNet20007Resp(data)
    ...
    if showType ~= 0 then  --不是0的，则有对应的动画 --构建数值
        ...
        checkShield(GamePowerConfig.Hero, info.heros, "onlyOne")
        checkShield(GamePowerConfig.Ordnance, info.odInfos, "onlyOne")
        checkShield(GamePowerConfig.HeroFragment, info.heroPieceInfos)

        -- index = self:setDataRewardFly(info.soldierList, tempData, index, GamePowerConfig.Soldier) --佣兵406
        -- index = self:setDataRewardFly(info.itemList, tempData, index, GamePowerConfig.Item)   --道具401
        -- index = self:setDataRewardFly(info.treasureInfos, tempData, index, GamePowerConfig.HeroTreasure,"onlyOne")   --宝具410
        -- index = self:setDataRewardFly(info.treasurePieceInfos, tempData, index, GamePowerConfig.HeroTreasureFragment)   --宝具碎片411
        -- index = self:setDataRewardFly(info.odpInfos, tempData, index, GamePowerConfig.OrdnanceFragment)   --军械碎片404
        -- index = self:setDataRewardFly(info.equipinfos, tempData, index, GamePowerConfig.General,"onlyOne")  --武将  402      没有num字段 数量为1
        -- index = self:setDataRewardFly(info.heros, tempData, index, GamePowerConfig.Hero,"onlyOne")  --武将  409      没有num字段 数量为1
        -- index = self:setDataRewardFly(info.odInfos, tempData, index, GamePowerConfig.Ordnance,"onlyOne")    --军械 403      没有num字段 数量为1
        -- index = self:setDataRewardFly(info.heroPieceInfos, tempData, index, GamePowerConfig.HeroFragment)   --武将碎片412
        if rawget(info, "diffs") then
            -- index = self:setDataRewardFly(info.diffs, tempData, index, GamePowerConfig.Resource, "value") --num字段为value
            ...
            if hero.heroId > 0 then
                ...
                if not isExpCar then
                    ...
                    if heroData == nil then
                        table.insert(heroIdList, hero.heroId)
                    end
                end
            end
        end
...
--弹出购买体力的弹窗
function RoleProxy:getBuyEnergyBox(panel,isShowMsgBox)
    ...
    local content = string.format(TextWords:getTextWord(507), price)
    self:showMessageBox(content, callByEnergy)

end

--弹窗购买讨伐令
function RoleProxy:getBuyCrusadeEnergyBox(panel,isShowMsgBox)
...
function RoleProxy:setDataRewardFly(infos, tempData, index, powerId, numString)
    ...
    for _,info in pairs(infos) do
        ...
        local typeid = powerId == 409 and info.heroId or info.typeid

        local config = ConfigDataManager:getConfigByPowerAndID(powerId, typeid)
        if config ~= nil and config.showControl == 1 then
...
function RoleProxy:updateRoleRemainTime(diff)
    ...
    elseif diff.typeid == powerTab[4] and diff.value < 10 then --讨伐令不满
        ...
        if info.need > CommandLv then
            if isShowMsg then
                if customMsg then
                    ...
                    self:showSysMessage(string.format(TextWords:getTextWord(340001),info.need,info.name))
                end
            end
            return false  --未开启
        end
