...
function SettingProxy:initSettingConf()
	...
	local initSettingConf = {
		{id = 1, name = 1412, type = 1, status = 1, isGloble = true, func = self.setBackGroundMusic, key = "BackGroundMusic"},
		{id = 2, name = 1413, type = 1, status = 1, isGloble = true, func = self.setkeySoundEffect, key = "keySoundEffect"},
...
function SettingProxy:onTriggerNet20803Resp(data)
	if data.rs==0 then
		self:showSysMessage( TextWords[1435] )
		self._headHas = data.headHas
	end
end
--更新获得挂件
function SettingProxy:onTriggerNet20804Resp(data)
	if data.rs==0 then
		self:showSysMessage( TextWords[1436] )
		self._pendantHas = data.pendantHas
	end
end
