...
function ExamActivityProxy:onTriggerNet370004Resp(data)
    if data.rs == 0 then
        --领取成功通知
        self:setStateOfProvExamReward(2)
...
function ExamActivityProxy:setProvExamInfo(data)
    ...
    if data.hasReward ~= nil and self.provExamInfo.hasReward ~= nil then
        self:setStateOfProvExamReward(data.hasReward)
    end
end
--总的殿试信息
...
function ExamActivityProxy:getProvEaxmAllRewardArray()
    local config = ConfigDataManager:getInfosFilterByOneKey(ConfigData.CIntegralRewardConfig, "integralreward", self.provExamCtrlConfig.rankID)
    return config
end
--返回殿试所有段位奖励列表
function ExamActivityProxy:getPalaceEaxmAllRewardArray()
    local config = ConfigDataManager:getInfosFilterByOneKey(ConfigData.CRankingRewardConfig, "rankingreward", self.palaceExamCtrlConfig.rankID)
    return config
end
--主动修改乡试领取状态
function ExamActivityProxy:setStateOfProvExamReward(num)
    self.provExamInfo.hasReward = num
    self:sendNotification(AppEvent.PROXY_PROVEXAM_REWARD_UPDATE,num)
end
--主动修改殿试领取状态
...
function ExamActivityProxy:getStateOfProvExamReward()
    return self.provExamInfo.hasReward
end
