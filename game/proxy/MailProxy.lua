...
------------网络数据请求与同步-------------
function MailProxy:onTriggerNet160000Req(data)
...
function MailProxy:initSyncData(data)
    ...
    self._collectData = data.collectMails
...
--    local collectId = data.collectId -- 收藏原本id
--    local normalId  = data.normalId  -- 普通原本id
    
--    -- 修改普通邮件的链接id 为 0， 表示未收藏， 
--    -- 在普通总表里找到，改变链接id为0， 没有则不操作
--    for k , v in pairs( self._allMailList.shortInfos) do
--        if k == normalId then
--            self._allMailList.shortInfos[data.normalId].collectId = StringUtils:int32ToFixed64(0)
--            break
--        end
--    end
--    print("成功进行了取消")
