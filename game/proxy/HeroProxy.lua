...
function HeroProxy:onTriggerNet300005Resp(data)
	if data.rs == 0 then
		self:showSysMessage(TextWords:TextWords(290060))
	end
...
function HeroProxy:getHeroStarUpAttr(data, nextData)
	local baseData = self:getHeroAllAttr(data)
	local addKeys = {"hpMax", "hitRate", "criRate", "atk", "dodgeRate", "defRate"}
	local comNum = self.proxy:getRoleAttrValue(PlayerPowerDefine.POWER_command)
	for k,v in pairs(baseData) do
		if nextData ~= nil then
			v.add = nextData[addKeys[k]]
		else
			v.add = 0
		end
		if k == 1 or k == 4 then
...
--跳转到heroinfopanel的时候有时候没带数据，来这里拿
...
function HeroProxy:getHeroFight(pos, adviserInfo)
    ...
    logger:error("获得一个英雄的战力====%d", (hp + attack + hit + dodge + defrate + crit) / 100 * command)
	return ((hit + dodge + defrate + crit) / 100 + hp + attack) * command
end
