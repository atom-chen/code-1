...
function RebelsProxy:getRichTextInfoByKillInfo(killInfo)
    ...
    for i = 1, 3 do
        if i ~= 1 then
            table.insert(str, { " / ", ColorUtils.tipSize20, ColorUtils:getRichColorByQuality(1) })
        end
        -- 颜色从绿色开始
...
function RebelsProxy:getRewardDataList(rankType)
    ...
    local rankRewardId = nil

    -- 取第一条记录就可以
    if rankType == RebelsProxy.RANK_TYPE_PLAYER then
        rankRewardId = rewardDataList[1].personRankId
    else
        rankRewardId = rewardDataList[1].legionRankId
    end

    local rewardDataList = ConfigDataManager:getInfosFilterByOneKey(ConfigData.CRankingRewardConfig, "rankingreward", rankRewardId)
...
function RebelsProxy:onTriggerNet400002Resp(data)
    ...
    self:sendNotification(AppEvent.PROXY_REBELS_REWARD_UPDATE, { })
end
