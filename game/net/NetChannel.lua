...
function NetChannel:initProtobuf()
    ...
    local moduleIdList = {1,10,28,3,4,5,6,7,16,8,26,12,23,9,20,14,21,17,31,32,33,22,36,38,30,35,37,34,39,27,20,2}
...
function NetChannel:sendReconnectLogin()
    ...
    data["obj"] = {account = GameConfig.accountName, type = 2, areId = areId}
...
--    buffer = table.concat(testData,"")
    ...
    if GameConfig.packageInfo ~= 2 then
        logger:info("=====no==error==c--->s(%d %d)==:%d==", mId, cmdId, reqTime)
    end
