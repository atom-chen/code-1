...
--        roleProxy:setActorInfo(actorInfo)
    ...
    elseif rs == -2 then --被封号了！
        ...
        local box = self:showMessageBox(TextWords:getTextWord(14), nil, nil,nil,nil, parent)
...
--            systemProxy:onTriggerNet8888Req({})
    ...
    if detalTime > 120 * 1 then  --2分钟直接踢下线
        if GameConfig.isConnected == true and  GameConfig.isInitRoleInfo == true then --GameConfig.debug == false  and  or true 客户端网络状态还是连接的，但是已经没有心跳了
...
function SceneState:preLoadModuleAfterRoleInfo()
    local  loadAry = {}
    table.insert(loadAry, {info = "加载资源2", module = ModuleName.ActivityModule} )
