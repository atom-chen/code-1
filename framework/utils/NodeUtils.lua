...
--        listView:setVisible(true)
    ...
    local startWPos = cc.p(listViewWorldPos.x, downWorldPos.y + downSize.height * scale + distanceY * scale)
...
function NodeUtils:removeSwallow()
    local swallowLayer = NodeUtils:getTopViewByName("swallowLayer")
...
------
-- 截全屏 TODO该接口遇到listView等控件会产生截屏绿白色
-- @param  args [obj] 参数
...
function NodeUtils:nodeBlur(node, blurRadius, sampleNum)
    ...
    --local fragSource = fileUtiles:getStringFromFile("shaders/example_Blur_winrt.fsh")
    ...
    node:getGLProgramState():setUniformFloat("blurRadius", tonumber(sampleNum) or 10.0)
    node:getGLProgramState():setUniformFloat("sampleNum", tonumber(sampleNum)  or 5)
