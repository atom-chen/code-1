...
function TimerManager.start()
	...
	TimerManager._allFunc = {}
	TimerManager._running = true

	TimerManager._createTimerThread()

	_funcHandle = cc.Director:getInstance():getScheduler():scheduleScriptFunc(TimerManager.onEnterFrame, 0, false)
...
function TimerManager.stop()
	...
	if not _funcHandle then
		return
	end

	cc.Director:getInstance():getScheduler():unscheduleScriptEntry(_funcHandle)
...
function TimerManager.addTimer(time, callFunc, loop, data, token)
	...
	table.insert(TimerManager._allFunc , callFunc)
...
function TimerManager.clear()
	for k,v in pairs(TimerManager._allFunc) do
