...
function SocketTCP:close( ... )
	...
	Notifier.dispatch(SocketTCP.EVENT_CLOSE)
end
...
function SocketTCP:_disconnect()
	...
	Notifier.dispatch(SocketTCP.EVENT_CLOSED)
...
function SocketTCP:_onConnected()
	...
	Notifier.dispatch(SocketTCP.EVENT_CONNECTED)
	if self.connectTimeTickScheduler then scheduler.unscheduleGlobal(self.connectTimeTickScheduler) end
	...
	local __tick = function()
		while true do
			...
			then
				-- Notifier.dispatch(SocketTCP.EVENT_DATA, {data=(__partial or __body), partial=__partial, body=__body})
				return
			end
			if not __body and not __partial then
				return
			end
			...
			-- Notifier.dispatch(SocketTCP.EVENT_DATA, byteArray)
...
function SocketTCP:_connectFailure(status)
	...
	Notifier.dispatch(SocketTCP.EVENT_CONNECT_FAILURE)
	self:_reconnect();
end
